import requests


def test_get_key(host, username, password, request_timeout):
    url = host + '/keys/'
    r = requests.post(url, auth=(username, password), timeout=request_timeout)
    r.raise_for_status()
    key = r.json()['api-key']
    assert key != "", "no key returned"

    # delete key after tests are done
    delete_key_url = host + '/keys/' + key
    r_delete_key = requests.delete(delete_key_url, auth=(username, password))
    r_delete_key.raise_for_status()


def test_get_token(host, username, password, api_key, request_timeout):
    url = host + '/keys/' + api_key + '/tokens'
    r = requests.post(url, auth=(username, password), timeout=request_timeout)
    r.raise_for_status()
    token = r.json()['token']
    assert token != "", "no token returned"


def test_delete_token(host, username, password, api_key, request_timeout):
    # get new token
    get_token_url = host + '/keys/' + api_key + '/tokens'
    r_token = requests.post(get_token_url, auth=(username, password), timeout=request_timeout)
    r_token.raise_for_status()
    token = r_token.json()['token']
    assert token != "", "no token returned"

    # check if token works
    get_polyglot_alive = host + '/dap/alive'
    headers = {'Authorization': token}
    r_alive = requests.get(get_polyglot_alive, headers=headers, timeout=request_timeout)
    r_alive.raise_for_status()
    print("Get alive with token reponse: " + r_alive.text)

    # delete token
    delete_token_url = host + '/tokens/' + token
    r_delete_token = requests.delete(delete_token_url, auth=(username, password), timeout=request_timeout)
    r_delete_token.raise_for_status()
    print("Delete token reponse: " + r_delete_token.text)

    # make sure token is deleted
    headers = {'Authorization': token}
    r_alive_no_token = requests.get(get_polyglot_alive, headers=headers, timeout=request_timeout)
    print("Get alive invalid token reponse: " + r_alive_no_token.text)
    assert r_alive_no_token.status_code == 403, "Did not receive 403 (Forbidden)"
