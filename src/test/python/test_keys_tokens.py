...
def test_delete_token(host, username, password, api_key):
    # get new token
    get_token_url = host + '/keys/' + api_key + '/tokens'
    print("POST " + get_token_url)
    r_token = requests.post(get_token_url, auth=(username, password))
    ...
    r_alive = requests.get(get_polyglot_alive, headers=headers)
    ...
    headers = {'Authorization': token}
    r_alive_no_token = requests.get(get_polyglot_alive, headers=headers)
